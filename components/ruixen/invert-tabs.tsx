...
interface InvertTabsProps {
  ...
  sound?: boolean;
  className?: string;
  /** Stretch tabs evenly across the bar width */
  fill?: boolean;
  /** Nav-style dividers between tab buttons */
  dividers?: boolean;
}
...
export function InvertTabs({
  ...
  return (
    <div className={`it${className ? ` ${className}` : ""}`}>
      ...
      <div
        ref={barRef}
        className={fill ? "it-bar it-bar--fill" : "it-bar"}
        style={{
          position: "relative",
      ...
      >
        {items.map((item, i) => (
          <Fragment key={item.value}>
            {dividers && i > 0 ? (
              <span className="it-bar__divider" aria-hidden="true" />
            ) : null}
            <button
              ref={(el) => {
              ...
              style={{
                ...
                border: "none",
                background: "none",
                padding: fill ? "0 10px" : "0 14px",
                fontSize: fill ? 13 : 14,
                fontWeight: fill ? 600 : 500,
                fontFamily: "inherit",
                color: "var(--it-text)",
                cursor: "pointer",
                whiteSpace: "nowrap",
