...
function Tile({
  ...
  if (reduce) {
    return (
      <figure ref={ref} className="clients-arch-grid__tile relative z-10 m-0">
        <div
        ...
        >
          ...
          />
          {caption ? (
            <div className="clients-arch-grid__caption">
              <div className="clients-arch-grid__name">
                {caption.name}
                {caption.nameEmphasis ? <em>{caption.nameEmphasis}</em> : null}
              </div>
              {caption.meta ? (
                <div className="clients-arch-grid__meta">{caption.meta}</div>
              ) : null}
            </div>
          ) : null}
        </div>
  ...
  return (
    <motion.figure
      ref={ref}
      className="clients-arch-grid__tile relative z-10 m-0"
...
export type ScrollTiltedGridProps = {
  ...
  /** Image tiles with optional captions. Takes precedence over `images`. */
...
export function ScrollTiltedGrid({
  ...
  const sourceTiles = useMemo<GridTile[]>(
    () =>
      tiles?.length
        ? [...tiles]
        : images.map((src) => ({ src })),
  ...
  const items = useMemo(
    () =>
      loop
        ? Array.from({ length: cycles }, () => sourceTiles).flat()
        : [...sourceTiles],
    [loop, cycles, sourceTiles],
  );
